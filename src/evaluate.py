...
def assess_model(y_pred, y_true, labels, target_names, labels_with_o, target_names_with_o, dataset_type,
    ...
    for f1_average_style in ['weighted', 'micro', 'macro']:
        results['f1_score'][f1_average_style] = sklearn.metrics.f1_score(y_true, y_pred, average=f1_average_style,
                                                                         labels=labels) * 100
    results['f1_score']['per_label'] = [x * 100 for x in
                                        sklearn.metrics.precision_recall_fscore_support(y_true, y_pred, average=None,
...
def plot_f1_vs_epoch(results, stats_graph_folder, metric, parameters, from_json=False):
    ...
    for dataset_type in dataset_types:
        ...
        plot_handles.extend(plt.plot(epoch_idxs, f1, '-', label=dataset_type + ' (max: {0:.4f})'.format(
    ...
    for dataset_type in dataset_types:
        ...
        if dataset_type == 'test':
            ...
            plt.axhline(y=best_score_based_on_valid, label='{0:.4f}'.format(best_score_based_on_valid), color='k',
                        linestyle=':')
    ...
    plt.savefig(
        os.path.join(stats_graph_folder, '{0}_vs_epoch_for_all_classes.{1}'.format(metric, parameters['plot_format'])))
    plt.close()
...
def evaluate_model(results, dataset, y_pred_all, y_true_all, stats_graph_folder, epoch_number,
                   epoch_start_time, output_filepaths, parameters, verbose=False):
    ...
    for dataset_type in ['train', 'valid', 'test']:
        ...
        for evaluation_mode in ['bio', 'token', 'binary']:
            y_pred, y_true, label_indices, label_names, label_indices_with_o, label_names_with_o = remap_labels(
                y_pred_original, y_true_original, dataset,
                evaluation_mode=evaluation_mode)
            result_update[dataset_type][evaluation_mode] = assess_model(y_pred, y_true, label_indices, label_names,
                                                                        label_indices_with_o, label_names_with_o,
                                                                        dataset_type, stats_graph_folder, epoch_number,
                                                                        parameters, evaluation_mode=evaluation_mode,
                                                                        verbose=verbose)
    ...
    for dataset_type in ['train', 'valid', 'test']:
        ...
        shell_command = '/usr/bin/perl {0} < {1} > {2}'.format(conll_evaluation_script, output_filepaths[dataset_type],
