...
def attach_labels(labels, lines):
    ...
    assert len(labels) == len(lines), "Number of labels (%d) does not match number of lines (%d)" % (
    len(labels), len(lines))
    ...
    for i, l in enumerate(lines):
        ...
        for o in range(start, end):
            if o in offset_label:
                if o != start:
                    print >> sys.stderr, 'Warning: annotation-token boundary mismatch: "%s" --- "%s"' % (
