...
def generate_reference_text_file_for_conll(conll_input_filepath, conll_output_filepath, text_folder):
    ...
    conll_file = codecs.open(conll_input_filepath, 'r', encoding='latin-1', errors='replace')
    ...
    for line in conll_file:
        ...
        if '-DOCSTART-' in split_line[0]:
            ...
            if len(text) != 0:
                with codecs.open(os.path.join(text_folder, '{0}.txt'.format(text_base_filename)), 'w', 'latin-1',
                                 errors='replace') as f:
                    f.write(text)
            text = ''
            character_index = 0
    ...
    with codecs.open(conll_output_filepath, 'w', 'latin-1', errors='replace') as f:
...
def check_compatibility_between_conll_and_brat_text(conll_filepath, brat_folder):
    ...
    conll_file = codecs.open(conll_filepath, encoding='latin-1', errors='replace')
...
def output_entities(brat_output_folder, previous_filename, entities, text_filepath, text, overwrite=False):
    ...
    with codecs.open(output_filepath, 'w', 'latin-1', errors='replace') as output_file:
