...
def plot_weights(weight_matrix, weight_filepath):
    plt.imshow(weight_matrix, cmap='hot', interpolation='nearest')
    plt.savefig(weight_filepath, dpi=300, format='png', bbox_inches='tight')
...
def plot_f1(f1s, graph_path, title):
    ...
    plt.savefig(graph_path, dpi=600, format='svg', bbox_inches='tight') # use format='svg' or 'pdf' for vectorial pictures
    plt.close()

def main():
    f1_dict = {'train-F1': [90]*30}
    plot_f1(f1_dict, './test.svg', 'test')
