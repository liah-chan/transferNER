...
def process(fn):
    ...
    try:
        #tagfile = open(fn)
        tagfile = codecs.open(fn, "rt", "latin-1")
    except:
    ...
    for ln, l in enumerate(tagtext.split('\n')):
        ...
        fields = l.split()
        assert len(fields) == 4, "Error: expected 7 tab-separated fields on line %d in %s, found %d: %s" % (ln+1, fn, len(fields), l.encode("latin-1"))
...
#                 reffile = codecs.open(reffn, "rt", "latin-1")
...
#                 out = codecs.open(outfn, "wt", "latin-1")
                out = open(outfn, "wt")
#             except:
...
#             reffile.close()
        ...
        try:
            assert reftext[start:end] == ttext, "ERROR: text mismatch for %s on line %d: reference '%s' tagged '%s': %s" % (fn, ln+1, reftext[start:end].encode("latin-1"), ttext.encode("latin-1"), l.encode("latin-1"))
        except:
            print("reftext[start:end]: {0}".format(reftext[start:end].encode("latin-1")))
            print("ttext: {0}".format(ttext.encode("latin-1")))
...
#         assert eText == eText.strip(), "ERROR: entity contains extra whitespace in %s: '%s'" % (fn, eText)
    ...
    for startoff, endoff, ttag, ttype in taggedTokens:
        ...
        if prevTag != "O" and ttag != "I":
            ...
            print(entityStr(currStart, prevEnd, currType, idIdx, reftext).encode("latin-1"))
            print >> out, entityStr(currStart, prevEnd, currType, idIdx, reftext).encode("latin-1")
...
def get_dataset_folder_original(model='crf'):
    if model=='ann':
        ...
        return os.path.join('/hardmnt/amosoz0/home/lchen', 'data', 'datasets', 'original')
    elif model == 'crf':
        # return os.path.join('..', '..', 'ner-deid', 'data', 'datasets', 'original')
        return os.path.join('/hardmnt/amosoz0/home/lchen','ner-deid', 'data', 'datasets', 'original')
