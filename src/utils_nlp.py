...
def remove_bio_from_label_name(label_name):
    if label_name[:2] in ['B-', 'I-', 'E-', 'S-']:
        ...
        assert(label_name == 'O' or label_name == 'OalInformation')
...
def convert_conll_from_bio_to_bioes(input_conll_filepath, output_conll_filepath):
    ...
    input_conll_file = codecs.open(input_conll_filepath,"r",encoding='latin-1', errors='replace')
    output_conll_file = codecs.open(output_conll_filepath, 'w', 'latin-1', errors='replace')
